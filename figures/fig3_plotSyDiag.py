...
Compare Sy from the truth and the predictions of the three neural
...
# load data
preds1 = np.load('../data/Predictions/Sy_Pred_R1_str2.npz')
preds2 = np.load('../data/Predictions/Sy_Pred_R2_str2.npz')
preds3 = np.load('../data/Predictions/Sy_Pred_R3_str2.npz')
...
for i in range(512) :  # loop through x
    ...
    for j in range(512) :  # loop throughy y
        ...
        r[0,j,i] = np.corrcoef( np.squeeze( SyTrue[:,j,i] ), np.squeeze( SyPred1[:,j,i] ), rowvar=False )[0,1]
        r[1,j,i] = np.corrcoef( np.squeeze( SyTrue[:,j,i] ), np.squeeze( SyPred2[:,j,i] ), rowvar=False )[0,1]
        r[2,j,i] = np.corrcoef( np.squeeze( SyTrue[:,j,i] ), np.squeeze( SyPred3[:,j,i] ), rowvar=False )[0,1]



##### Plotting: Sy Predictions #####
...
# True Sy
im0 = axArr[0,0].imshow( np.squeeze( SyTrue[t0,:,:] )*1e6, origin='lower', cmap='seismic', vmin=-cLim, vmax=cLim )
axArr[0,0].set_title('True $S_y$ Snapshot', fontsize=9 )
...
# Predicted Sy (training region 1)
im1 = axArr[0,1].imshow( np.squeeze( SyPred1[t0,:,:] )*1e6, origin='lower', cmap='seismic', vmin=-cLim, vmax=cLim )
axArr[0,1].set_title(r'Snapshot $f_y(\overline{\psi},\mathbf{w}_1)$', fontsize=9 )
...
# Predicted Sy (training region 2)
im2 = axArr[0,2].imshow( np.squeeze( SyPred2[t0,:,:] )*1e6, origin='lower', cmap='seismic', vmin=-cLim, vmax=cLim )
axArr[0,2].set_title(r'Snapshot $f_y(\overline{\psi},\mathbf{w}_2)$', fontsize=9 )
...
# Predicted Sy (training region 3)
im3 = axArr[0,3].imshow( np.squeeze( SyPred3[t0,:,:] )*1e6, origin='lower', cmap='seismic', vmin=-cLim, vmax=cLim )
axArr[0,3].set_title(r'Snapshot $f_y(\overline{\psi},\mathbf{w}_3)$', fontsize=9 )
...
# Mean True Sy
im4 = axArr[1,0].imshow( np.squeeze( np.mean( SyTrue, axis=0 ) )*1e6, origin='lower', cmap='seismic', vmin=-cLim, vmax=cLim )
axArr[1,0].set_title('Mean of True $S_y$', fontsize=9 )
...
# Mean Predicted Sy (training region 1)
im5 = axArr[1,1].imshow( np.squeeze( np.mean( SyPred1, axis=0 ) )*1e6, origin='lower', cmap='seismic', vmin=-cLim, vmax=cLim )
axArr[1,1].set_title(r'Mean $f_y(\overline{\psi},\mathbf{w}_1)$', fontsize=9 )
...
# Mean Predicted Sy (training region 2)
im6 = axArr[1,2].imshow( np.squeeze( np.mean( SyPred2, axis=0 ) )*1e6, origin='lower', cmap='seismic', vmin=-cLim, vmax=cLim )
axArr[1,2].set_title(r'Mean $f_y(\overline{\psi},\mathbf{w}_2)$', fontsize=9 )
...
# Mean Predicted Sy (training region 3)
im7 = axArr[1,3].imshow( np.squeeze( np.mean( SyPred3, axis=0 ) )*1e6, origin='lower', cmap='seismic', vmin=-cLim, vmax=cLim )
axArr[1,3].set_title(r'Mean $f_y(\overline{\psi},\mathbf{w}_3)$)', fontsize=9 )
...
# Std True Sy
im8 = axArr[2,0].imshow( np.squeeze( np.std( SyTrue, axis=0 ) )*1e6, origin='lower', cmap=cMap, vmin=0, vmax=cLim )
axArr[2,0].set_title('Std Dev of True $S_y$', fontsize=9 )
...
im9 = axArr[2,1].imshow( np.squeeze( np.std( SyPred1, axis=0 ) )*1e6, origin='lower', cmap=cMap, vmin=0, vmax=cLim )
axArr[2,1].set_title(r'Std Dev $f_y(\overline{\psi},\mathbf{w}_1)$', fontsize=9 )
...
im10 = axArr[2,2].imshow( np.squeeze( np.std( SyPred2, axis=0 ) )*1e6, origin='lower', cmap=cMap, vmin=0, vmax=cLim )
axArr[2,2].set_title(r'Std Dev $f_y(\overline{\psi},\mathbf{w}_2)$', fontsize=9 )
...
im11 = axArr[2,3].imshow( np.squeeze( np.std( SyPred3, axis=0 ) )*1e6, origin='lower', cmap=cMap, vmin=0, vmax=cLim )
axArr[2,3].set_title(r'Std Dev $f_y(\overline{\psi},\mathbf{w}_3)$', fontsize=9 )
...
# Corr Predicted Sy (training region 1)
im13 = axArr[3,1].imshow( np.squeeze( r[0,:,:] ), origin='lower', cmap=cMap, vmin=-1, vmax=1 )
axArr[3,1].set_title(r'Corr($S_y,\tilde{S}_y$): $f_y(\overline{\psi},\mathbf{w}_1)$', fontsize=9 )
...
# Corr Predicted Sy (training region 2)
im14 = axArr[3,2].imshow( np.squeeze( r[1,:,:] ), origin='lower', cmap=cMap, vmin=-1, vmax=1 )
axArr[3,2].set_title(r'Corr($S_y,\tilde{S}_y$): $f_y(\overline{\psi},\mathbf{w}_2)$', fontsize=9 )
...
# Corr Predicted Sy (training region 3)
im15 = axArr[3,3].imshow( np.squeeze( r[2,:,:] ), origin='lower', cmap=cMap, vmin=-1, vmax=1 )
axArr[3,3].set_title(r'Corr($S_y,\tilde{S}_y$): $f_y(\overline{\psi},\mathbf{w}_3)$', fontsize=9 )
...
cBar.ax.set_ylabel(r'$\tilde{S}_y$ (10$^{-6}$ms$^{-2}$)', fontsize=9 )
...
cBar.ax.set_ylabel(r'Time-Mean $\tilde{S}_y$ (10$^{-6}$ms$^{-2}$)', fontsize=9 )
cBar.ax.tick_params( labelsize=9 )
...
cBar.ax.set_ylabel(r'Std Dev $\tilde{S}_y$ (10$^{-6}$ms$^{-2}$)', fontsize=9  )
cBar.ax.tick_params( labelsize=9 )
...
plt.text( posX0.x0 + 0.5*posX0.width, 0.91, r'\textbf{Truth} $S_y$', ha='center', fontsize=12, fontweight='bold', transform=fig.transFigure )
plt.text( posX2.x0 + 0.5*posX2.width, 0.95, r'\textbf{Convolutional Neural Networks} $\tilde{S}_y = f_y(\overline{\psi},\mathbf{w}_R)$', ha='center', fontsize=12, fontweight='bold', transform=fig.transFigure )
...
plt.savefig('SyPredictions.png',format='png',dpi=300 )
